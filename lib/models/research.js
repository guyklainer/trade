	...
	lookForBuy(){
		...
		if(percent < (this.avgs.avg_ask-2)){
			if(sellValue <= available){
				...
				console.log( `Can ${chalk.green("BUY")} amount of ${chalk.yellow(potentialSeller.amount)} in ${chalk.green(potentialSeller.price)}. It's ${chalk.blue(discount)}(${percent}%) NIS below the market price and ${this.avgs.avg_ask-percent} below the AVG sell price which is ${this.avgs.avg_ask}`);

			} else {
				this.actions.push(new Trade("BUY", potentialSeller.price, available / potentialSeller.price));
				console.log( `Can ${chalk.green("BUY")} amount of ${chalk.yellow(available / potentialSeller.price)} in ${chalk.green(potentialSeller.price)}. It's ${chalk.blue(discount)}(${percent}%) NIS below the market price and ${this.avgs.avg_ask-percent} below the AVG sell price which is ${this.avgs.avg_ask}`);
			}
		}
	}
	...
	lookForSell(){
		...
		if(percent > (this.avgs.avg_bid+2)){
			if(buyValue <= available){
				...
				console.log( `Can ${chalk.red("CELL")} amount of ${chalk.yellow(potentialBuyer.amount)} in ${chalk.red(potentialBuyer.price)}. It's ${chalk.blue(discount)}(${percent.toFixed(2)}%) NIS above the market price and ${percent-this.avgs.avg_bid} above the AVG buy price which is ${this.avgs.avg_bid}`);

			} else {
				this.actions.push(new Trade("CELL", potentialBuyer.price, available));
				console.log( `Can ${chalk.green("CELL")} amount of ${chalk.yellow(available)} in ${chalk.green(potentialBuyer.price)}. It's ${chalk.blue(discount)}(${percent.toFixed(2)}%) NIS above the market price and ${percent-this.avgs.avg_bid} above the AVG buy price which is ${this.avgs.avg_bid}`);
			}
		}
	}
