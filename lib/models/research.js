...
let skipMarked = {bid: false, ask: {skip: false, insufficient: false}};
...
class Research {
	...
	lookForBuy(){
		...
		const availableAmount = available / potentialSeller.price;
		...
		if(percent < (this.avgs.avg_ask-2)){
			...
			} else if(!skipMarked.ask.insufficient){
				skipMarked.ask.insufficient = true;
		...
		} else if(percent < (this.avgs.avg_ask-1)){
			if(!skipMarked.ask.skip){
				skipMarked.ask.skip = true;
			...
			skipMarked.ask.skip = false;
			skipMarked.ask.insufficient = false;
		}
	}
	...
	lookForSell(){
		...
		if(percent > (this.avgs.avg_bid+2)){
			if(buyValue <= available && potentialBuyer.amount > 0){
		...
		} else if(percent > (this.avgs.avg_bid+1)){
			if(!skipMarked.bid){
				skipMarked.bid = true;
				const msg = `*Skipped ${chalk.green("CELL")}* amount of ${chalk.yellow(available)} in ${chalk.green(potentialBuyer.price)}. It's ${chalk.blue(discount)}(${percent.toFixed(2)}%) NIS above the market price and *${percent-this.avgs.avg_bid}* above the AVG buy price which is ${this.avgs.avg_bid}`;
				this._publish(msg);
			}
