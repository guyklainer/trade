...
class Research {
	...
	lookForBuy(){
		...
		if(percent < (this.avgs.avg_ask-2)){
			if(sellValue <= available){
			...
			} else if(amount*potentialSeller.price > 10){
				...
				if(amount > 0){
					...
					console.log( `[${this.stats.time}][${this.requestID}] Can ${chalk.green("BUY")} amount of ${chalk.yellow(amount)} in ${chalk.green(potentialSeller.price)}. It's ${chalk.blue(discount)}(${percent}%) NIS below the market price and ${this.avgs.avg_ask-percent} below the AVG sell price which is ${this.avgs.avg_ask}`);
				}
			}
		}
	}
