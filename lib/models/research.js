	...
	lookForBuy(){
		...
		if(percent < (this.avgs.avg_ask-2)){
			if(sellValue <= available){
				...
				const amount = parseFloat((available / potentialSeller.price).toFixed(4));

				if(amount > 0){
					this.actions.push(new Trade("BUY", potentialSeller.price, amount));
					const msg = `*Can ${chalk.green("BUY")}* amount of ${chalk.yellow(amount)} in ${chalk.green(potentialSeller.price)}. It's ${chalk.blue(discount)}(${percent}%) NIS below the market price and *${this.avgs.avg_ask-percent}* below the AVG sell price which is ${this.avgs.avg_ask}`;
					this._publish(msg);
				}

			} else {
				skipMarked.ask = true;
				const msg = `*Insufficient funds for ${chalk.green("BUY")}* amount of ${chalk.yellow(amount)} in ${chalk.green(potentialSeller.price)}. It's ${chalk.blue(discount)}(${percent}%) NIS below the market price and *${this.avgs.avg_ask-percent}* below the AVG sell price which is ${this.avgs.avg_ask}`;
				this._publish(msg);
			}
			...
			if(!skipMarked.ask){
				skipMarked.ask = true;
				const msg = `*Skipped ${chalk.green("BUY")}* amount of ${chalk.yellow(amount)} in ${chalk.green(potentialSeller.price)}. It's ${chalk.blue(discount)}(${percent}%) NIS below the market price and *${this.avgs.avg_ask-percent}* below the AVG sell price which is ${this.avgs.avg_ask}`;
				this._publish(msg);
			}
