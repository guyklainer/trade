	...
	addOrder(){
		...
		if(trade.isValid()){
			...
			return bit2c.addOrderAsync(this.context.creds, trade.getRequest())
				.then(res => {
					if(res.OrderResponse && res.OrderResponse.HasError)
						return Promise.reject(res.OrderResponse.Error);
					else
						debug(`${this.prefix} add_order_success: ${JSON.stringify(trade.getRequest())}`);
				})
				.catch(err => {
					debug(`${this.prefix} add_order_failed: ${JSON.stringify(trade.getRequest())} ${err}`);
				});
