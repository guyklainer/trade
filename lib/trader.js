...
class Trader {
	...
	work(){
		...
		return Promise.all(calls).then( ([bitstamp, bit2c, bit2cOrders, balance, rates]) => {
			...
			if( action == "BUY")
				values.push("BUY", buyOptions.count, buyOptions.price, buyOptions.sum, (100*(bit2cOrders.asks[0][0]/bitstampInNIS))-100, (100*(bit2cOrders.bids[0][0]/bitstampInNIS))-100);
			else if( action == "SELL")
				values.push("'SELL'", sellOptions.count, sellOptions.price, sellOptions.sum, (100*(bit2cOrders.asks[0][0]/bitstampInNIS))-100, (100*(bit2cOrders.bids[0][0]/bitstampInNIS))-100);
			else if ( sellOptions.sum == -1 )
				values.push("'POTENTIAL SELL'", sellOptions.count, sellOptions.price, sellOptions.count*sellOptions.price, 0, (100*(bit2cOrders.asks[0][0]/bitstampInNIS))-100, (100*(bit2cOrders.bids[0][0]/bitstampInNIS))-100);
			else if ( buyOptions.sum == -1 )
				values.push("'POTENTIAL BUY'", buyOptions.count, buyOptions.price, buyOptions.count*buyOptions.price, 0, (100*(bit2cOrders.asks[0][0]/bitstampInNIS))-100, (100*(bit2cOrders.bids[0][0]/bitstampInNIS))-100);
			else
				values.push("NULL", "NULL", "NULL", "NULL", "NULL", "NULL");

			values.push(`'${moment().format('YYYY-MM-DD HH:mm:ss')}'`);
			this.db.insert(values, _ => values.splice(0));
