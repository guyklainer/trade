	...
	work(){
		...
		return Promise.all(calls).then( ([bitstamp, bit2c, bit2cOrders, balance, rates]) => {
			...
			if( action == "BUY")
				...
				values.push("'POTENTIAL SELL'", sellOptions.count, sellOptions.price, sellOptions.count*sellOptions.price, 0);
			else if ( buyOptions.sum == -1 )
				values.push("'POTENTIAL BUY'", buyOptions.count, buyOptions.price, buyOptions.count*buyOptions.price, 0);
			else
